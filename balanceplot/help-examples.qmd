...
The examples at the end of `help balanceplot`, run in order with their output
and graphs. The [Options](options.html), [Continuous
treatments](continuous-treatment.html), and [Matching and
teffects](matching.html) pages explain the options in more detail.
...
dropped as the nominal categories are no longer together on the graph.
...
imbalance, and its p-value -- alongside the graph. The same numbers are
returned in `r(table1)` whether or not a table is printed.
...
(`bp_mean_g0`, `bp_mean_g1`, `bp_imbalance`) so that `esttab` can lay them out
as three columns.
...
imbalance result per nonbase comparison (`br_imb_g2`, `br_imb_g3`).
...
so each can be passed to `esttab` on its own.
