...
and once with `scheme(s2color)`, Stata's long-standing default scheme (Stata
18's default is `stcolor`). Nothing else about the commands changes. The
graphs are drawn with the data from [`usetdm`](../usetdm/index.html), so every
example can be run as shown. With `set scheme cleanplots, perm` in place, the
`scheme()` option is not needed.
