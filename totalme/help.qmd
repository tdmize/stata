...
| **2sd** | A two standard deviation change (Gelman 2008), useful for comparing continuous effects to categorical effects; centered by default, from [x - SD] to [x + SD]. **twosd** is a synonym |
| **trimrange** | A change across the trimmed range of the variable, from its 5th to its 95th percentile in the estimation sample (Mize and Han 2025); `start()` and centering do not apply |
| **range** | A change across the full observed range of the variable, from its minimum to its maximum in the estimation sample; `start()` and centering do not apply |
| **p***#***-p***#* | A change from one percentile of the variable to another in the estimation sample, e.g. **p10-p90** for its 10th to its 90th percentile; **trimrange** is **p5-p95**. `start()` and centering do not apply |
| **rate** | An instantaneous rate of change, i.e. the derivative, approximated with a small centered change; **slope** and **dydx** are synonyms. Centering does not apply |
...
Neither option may name one of the focal variables: `totalme` does not estimate the total ME of a variable within levels of that same variable and exits with an error; [mecompare](../mecompare/help.html) handles that case.
...
The scalars follow one naming scheme whether one or two models are given. In each name *#* is the variable's number within its type (the first continuous/binary variable is 1, the first nominal variable is 1). With `by()` or `over()`, `_`*level* is appended to every name, including the cross-model differences, and the Diff. rows append `_d`*level1*`_`*level2* instead (e.g. `r(tmcm11_d0_1)` is the total ME of the first continuous/binary variable at level 0 minus that at level 1).
