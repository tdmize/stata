...
![](../fig/nli-alcohol-predictions.png){width=75% fig-alt="Probability of alcohol use by gender and parental status, as bars with confidence intervals."}
...
drop is larger for women than for men (though the second difference is only 
marginally significant). The other side of the interaction is the gender gap 
for non-parents and for parents:
...
The ten predicted probabilities as a dot plot, education on the axis and one
marker per gender. As for Figure 12, the predictions for each gender are
posted and stored separately so that `coefplot` labels the levels of
education and lines the two genders up:
...
![](../fig/nli-alcohol-education.png){width=80% fig-alt="Probability of alcohol use by gender and educational attainment, one marker per gender at each level of education."}
